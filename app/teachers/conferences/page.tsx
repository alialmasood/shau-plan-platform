...
export default function ConferencesPage() {
  ...
  useEffect(() => {
    const fetchConferences = async () => {
      ...
      try {
        setIsLoading(true);
        const response = await fetch(`/api/teachers/conferences?userId=${user.id}`);
        if (response.ok) {
  ...
  const handleInputChange = (field: keyof Conference, value: any) => {
    setFormData((prev) => ({
      ...prev,
      [field]: value,
    }));
  ...
  const handleEditConference = (conference: Conference) => {
    ...
    setFormData({
      ...
      date: conference.date ? conference.date.split('T')[0] : "",
      scope: conference.scope || "local",
      type: conference.type || "attendance",
      sponsoring_organization: conference.sponsoring_organization || "",
      location: conference.location || "",
      is_committee_member: conference.is_committee_member || false,
      assignment_document: conference.assignment_document || "",
    });
    setFilePreview(conference.assignment_document || "");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = `/api/teachers/conferences`;

      const method = editingConference?.id ? "PATCH" : "POST";
      ...
      const body = editingConference?.id
        ? {
            id: editingConference.id,
            conferenceTitle: formData.conference_title,
            date: formData.date,
            scope: formData.scope,
            type: formData.type,
            sponsoringOrganization: formData.sponsoring_organization,
            location: formData.location,
            isCommitteeMember: formData.is_committee_member,
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || errorData.details || "فشل حفظ المؤتمر");
      }

      const result = await response.json();
      const savedConference = result.conference || result;

      if (editingConference?.id) {
        setConferences((prev) =>
          prev.map((c) => (c.id === savedConference.id ? savedConference : c))
        );
      } else {
        setConferences((prev) => [savedConference, ...prev]);
      }
      ...
      alert(error.message || "حدث خطأ أثناء حفظ المؤتمر");
    }
  };

  const handleDeleteConference = async () => {
    if (!conferenceToDelete?.id) return;

    try {
      const response = await fetch(`/api/teachers/conferences?id=${conferenceToDelete.id}`, {
        method: "DELETE",
      });

      if (response.ok) {
        setConferences((prev) => prev.filter((c) => c.id !== conferenceToDelete.id));
        setConferenceToDelete(null);
      } else {
        throw new Error("فشل حذف المؤتمر");
      }
    } catch (error) {
      console.error("Error deleting conference:", error);
      alert("حدث خطأ أثناء حذف المؤتمر");
    }
  };
    ...
    <div className="space-y-6">
      <div className="bg-white rounded-lg border border-gray-200 p-6 shadow-sm">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-2xl font-bold mb-2" style={{ color: '#1F2937' }}>المؤتمرات</h1>
        ...
        {isLoading ? (
          ...
          <div className="text-center py-12">
            ...
            <h3 className="mt-2 text-sm font-medium" style={{ color: '#374151' }}>لا توجد مؤتمرات</h3>
            <p className="mt-1 text-sm text-gray-500">ابدأ بإضافة مؤتمر جديد</p>
          </div>
        ) : (
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {conferences.map((conference) => (
                  <tr key={conference.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-4 py-4 whitespace-nowrap">
                      <div className="text-sm" style={{ color: '#374151' }}>
                        {getScopeLabel(conference.scope)}
                      </div>
                    </td>
                    <td className="px-4 py-4 whitespace-nowrap">
                      <div className="text-sm" style={{ color: '#374151' }}>
                        {getTypeLabel(conference.type)}
                      </div>
                    </td>
                    <td className="px-4 py-4 whitespace-nowrap">
                      <div className="text-sm" style={{ color: '#374151' }}>
                        {conference.sponsoring_organization || "-"}
                      </div>
                    </td>
                    <td className="px-4 py-4 whitespace-nowrap">
                      <div className="text-sm" style={{ color: '#374151' }}>
                    ...
                    <td className="px-4 py-4 whitespace-nowrap">
                      <div className="text-sm" style={{ color: '#374151' }}>
                        {conference.is_committee_member ? "نعم" : "لا"}
                      </div>
                    </td>
                    <td className="px-4 py-4 whitespace-nowrap">
                      {conference.assignment_document ? (
                        <div className="flex items-center gap-2">
                          {conference.assignment_document.startsWith('data:image/') ? (
                            <>
                              <img 
                                src={conference.assignment_document} 
                                alt="معاينة" 
                                className="w-12 h-12 object-cover rounded border border-gray-200 cursor-pointer hover:opacity-80 transition-opacity"
                                onClick={() => {
                                  const newWindow = window.open();
                                  if (newWindow) {
                                    newWindow.document.write(`<img src="${conference.assignment_document}" style="max-width: 100%; height: auto;" />`);
                                  }
                                }}
                              ...
                              <button
                                onClick={() => {
                                  ...
                                  link.href = conference.assignment_document!;
                                  link.download = `assignment_${conference.id || 'document'}.jpg`;
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              <button
                                onClick={() => {
                                  const newWindow = window.open();
                                  if (newWindow && conference.assignment_document) {
                                    newWindow.document.write(`
                                      <iframe src="${conference.assignment_document}" style="width: 100%; height: 100vh; border: none;"></iframe>
                                    `);
                                  }
                                }}
                              ...
                              <button
                                onClick={() => {
                                  const link = document.createElement('a');
                                  link.href = conference.assignment_document!;
                                  link.download = `assignment_${conference.id || 'document'}.pdf`;
                                  document.body.appendChild(link);
                                  link.click();
                                  document.body.removeChild(link);
                                }}
                    ...
                    <td className="px-4 py-4 whitespace-nowrap text-sm font-medium">
                      <div className="flex items-center gap-2">
                        ...
                        <button
                          onClick={() => setConferenceToDelete(conference)}
      ...
      {conferenceToDelete && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg p-6 max-w-md w-full">
            ...
            <p className="text-gray-600 mb-6">هل أنت متأكد من حذف المؤتمر "{conferenceToDelete.conference_title}"؟</p>
          ...
          <div className="bg-white rounded-lg p-6 max-w-2xl w-full max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-xl font-bold" style={{ color: '#1F2937' }}>
                {editingConference?.id ? "تعديل المؤتمر" : "إضافة مؤتمر جديد"}
              </h3>
              <button
                onClick={() => {
                  setShowForm(false);
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                <label className="block text-sm font-medium mb-2" style={{ color: '#374151' }}>
                  عنوان المؤتمر <span className="text-red-500">*</span>
                </label>
                <input
                  type="text"
                  value={formData.conference_title}
                  onChange={(e) => handleInputChange("conference_title", e.target.value)}
                  required
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => handleInputChange("sponsoring_organization", e.target.value)}
                  className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all duration-300"
                  style={{ color: '#1F2937', backgroundColor: '#FAFBFC' }}
                  placeholder="أدخل الجهة الراعية للمؤتمر"
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all duration-300"
                  style={{ color: '#1F2937', backgroundColor: '#FAFBFC' }}
                  placeholder="أدخل مكان انعقاد المؤتمر"
                />
              </div>
              ...
              <div>
                <label className="flex items-center gap-2 cursor-pointer">
                  <input
                    type="checkbox"
                    checked={formData.is_committee_member || false}
                    onChange={(e) => handleInputChange("is_committee_member", e.target.checked)}
              ...
              <div>
                <label className="block text-sm font-medium mb-2" style={{ color: '#374151' }}>
                  كتاب أو أمر المؤتمر (PDF أو صورة)
                </label>
              ...
              <div className="flex items-center justify-end gap-3 pt-4 border-t border-gray-200">
                ...
                >
                  {editingConference?.id ? "تحديث المؤتمر" : "إضافة المؤتمر"}
                </button>
              </div>
            </form>
          </div>
