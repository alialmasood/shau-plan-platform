...
export default function CoursesPage() {
  ...
  const handleEditCourse = (course: Course) => {
    ...
    setFormData({
      ...
      assignment_document: course.assignment_document || "",
    });
    setFilePreview(course.assignment_document || "");
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {courses.map((course) => (
                  <tr key={course.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-4 py-4 whitespace-nowrap">
                      {course.assignment_document ? (
                        <div className="flex items-center gap-2">
                          {course.assignment_document.startsWith('data:image/') ? (
                            <>
                              <img 
                                src={course.assignment_document} 
                                alt="معاينة" 
                                className="w-12 h-12 object-cover rounded border border-gray-200 cursor-pointer hover:opacity-80 transition-opacity"
                                onClick={() => {
                                  const newWindow = window.open();
                                  if (newWindow) {
                                    newWindow.document.write(`<img src="${course.assignment_document}" style="max-width: 100%; height: auto;" />`);
                                  }
                                }}
                              ...
                              <button
                                onClick={() => {
                                  ...
                                  link.href = course.assignment_document!;
                                  link.download = `assignment_${course.id || 'document'}.jpg`;
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              <button
                                onClick={() => {
                                  const newWindow = window.open();
                                  if (newWindow && course.assignment_document) {
                                    newWindow.document.write(`
                                      <iframe src="${course.assignment_document}" style="width: 100%; height: 100vh; border: none;"></iframe>
                                    `);
                                  }
                                }}
                              ...
                              <button
                                onClick={() => {
                                  const link = document.createElement('a');
                                  link.href = course.assignment_document!;
                                  link.download = `assignment_${course.id || 'document'}.pdf`;
                                  document.body.appendChild(link);
                                  link.click();
                                  document.body.removeChild(link);
                                }}
