...
export default function SeminarsPage() {
  ...
  const handleEditSeminar = (seminar: Seminar) => {
    ...
    setFormData({
      ...
      assignment_document: seminar.assignment_document || "",
    });
    setFilePreview(seminar.assignment_document || "");
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {seminars.map((seminar) => (
                  <tr key={seminar.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-4 py-4 whitespace-nowrap">
                      {seminar.assignment_document ? (
                        <div className="flex items-center gap-2">
                          {seminar.assignment_document.startsWith('data:image/') ? (
                            <>
                              <img 
                                src={seminar.assignment_document} 
                                alt="معاينة" 
                                className="w-12 h-12 object-cover rounded border border-gray-200 cursor-pointer hover:opacity-80 transition-opacity"
                                onClick={() => {
                                  const newWindow = window.open();
                                  if (newWindow) {
                                    newWindow.document.write(`<img src="${seminar.assignment_document}" style="max-width: 100%; height: auto;" />`);
                                  }
                                }}
                              ...
                              <button
                                onClick={() => {
                                  ...
                                  link.href = seminar.assignment_document!;
                                  link.download = `assignment_${seminar.id || 'document'}.jpg`;
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              <button
                                onClick={() => {
                                  const newWindow = window.open();
                                  if (newWindow && seminar.assignment_document) {
                                    newWindow.document.write(`
                                      <iframe src="${seminar.assignment_document}" style="width: 100%; height: 100vh; border: none;"></iframe>
                                    `);
                                  }
                                }}
                              ...
                              <button
                                onClick={() => {
                                  const link = document.createElement('a');
                                  link.href = seminar.assignment_document!;
                                  link.download = `assignment_${seminar.id || 'document'}.pdf`;
                                  document.body.appendChild(link);
                                  link.click();
                                  document.body.removeChild(link);
                                }}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4" onClick={() => setShowForm(false)}>
          <div className="bg-white rounded-lg p-6 max-w-2xl w-full max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium mb-2" style={{ color: '#374151' }}>
                  كتاب أو أمر الندوة (PDF أو صورة)
                </label>
