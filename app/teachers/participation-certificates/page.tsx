...
interface ParticipationCertificate {
  ...
  month?: string;
  year: number;
  certificate_document?: string; // base64 string for PDF/image
  created_at?: string;
  updated_at?: string;
}
...
export default function ParticipationCertificatesPage() {
  ...
  const handleEditCertificate = (certificate: ParticipationCertificate) => {
    ...
    setFormData({
      ...
      certificate_document: certificate.certificate_document || "",
    });
    setFilePreview(certificate.certificate_document || "");
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.type.startsWith('image/')) {
      ...
      reader.onloadend = () => {
        ...
        setFormData({ ...formData, certificate_document: base64String });
      };
  ...
  const handleRemoveFile = () => {
    setSelectedFile(null);
    setFilePreview("");
    setFormData({ ...formData, certificate_document: "" });
    // Reset file input
    const fileInput = document.getElementById('certificate-document') as HTMLInputElement;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const certificateDocument = filePreview || formData.certificate_document || null;
                    ...
                    <td className="px-4 py-4 whitespace-nowrap">
                      {certificate.certificate_document ? (
                        <div className="flex items-center gap-2">
                          {certificate.certificate_document.startsWith('data:image/') ? (
                            <>
                              <img 
                                src={certificate.certificate_document} 
                                alt="معاينة" 
                                className="w-12 h-12 object-cover rounded border border-gray-200 cursor-pointer hover:opacity-80 transition-opacity"
                                onClick={() => {
                                  const newWindow = window.open();
                                  if (newWindow) {
                                    newWindow.document.write(`<img src="${certificate.certificate_document}" style="max-width: 100%; height: auto;" />`);
                                  }
                                }}
                              ...
                              <button
                                onClick={() => {
                                  ...
                                  link.href = certificate.certificate_document!;
                                  link.download = `certificate_${certificate.id || 'document'}.jpg`;
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              <button
                                onClick={() => {
                                  const newWindow = window.open();
                                  if (newWindow && certificate.certificate_document) {
                                    newWindow.document.write(`
                                      <iframe src="${certificate.certificate_document}" style="width: 100%; height: 100vh; border: none;"></iframe>
                                    `);
                                  }
                                }}
                              ...
                              <button
                                onClick={() => {
                                  const link = document.createElement('a');
                                  link.href = certificate.certificate_document!;
                                  link.download = `certificate_${certificate.id || 'document'}.pdf`;
                                  document.body.appendChild(link);
                                  link.click();
                                  document.body.removeChild(link);
                                }}
