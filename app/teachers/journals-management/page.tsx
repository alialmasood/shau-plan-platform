...
interface JournalMembership {
  ...
  description?: string;
  is_active?: boolean;
  membership_document?: string; // base64 string for PDF/image
  created_at?: string;
  updated_at?: string;
}
...
export default function JournalsManagementPage() {
  ...
  const handleEditMembership = (membership: JournalMembership) => {
    ...
    setFormData({
      ...
      membership_document: membership.membership_document || "",
    });
    setFilePreview(membership.membership_document || "");
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.type.startsWith('image/')) {
      ...
      reader.onloadend = () => {
        ...
        setFormData({ ...formData, membership_document: base64String });
      };
  ...
  const handleRemoveFile = () => {
    setSelectedFile(null);
    setFilePreview("");
    setFormData({ ...formData, membership_document: "" });
    // Reset file input
    const fileInput = document.getElementById('membership-document') as HTMLInputElement;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const membershipDocument = filePreview || formData.membership_document || null;
                    ...
                    <td className="px-4 py-4 whitespace-nowrap">
                      {membership.membership_document ? (
                        <div className="flex items-center gap-2">
                          {membership.membership_document.startsWith('data:image/') ? (
                            <>
                              <img 
                                src={membership.membership_document} 
                                alt="معاينة" 
                                className="w-12 h-12 object-cover rounded border border-gray-200 cursor-pointer hover:opacity-80 transition-opacity"
                                onClick={() => {
                                  const newWindow = window.open();
                                  if (newWindow) {
                                    newWindow.document.write(`<img src="${membership.membership_document}" style="max-width: 100%; height: auto;" />`);
                                  }
                                }}
                              ...
                              <button
                                onClick={() => {
                                  ...
                                  link.href = membership.membership_document!;
                                  link.download = `membership_${membership.id || 'document'}.jpg`;
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              <button
                                onClick={() => {
                                  const newWindow = window.open();
                                  if (newWindow && membership.membership_document) {
                                    newWindow.document.write(`
                                      <iframe src="${membership.membership_document}" style="width: 100%; height: 100vh; border: none;"></iframe>
                                    `);
                                  }
                                }}
                              ...
                              <button
                                onClick={() => {
                                  const link = document.createElement('a');
                                  link.href = membership.membership_document!;
                                  link.download = `membership_${membership.id || 'document'}.pdf`;
                                  document.body.appendChild(link);
                                  link.click();
                                  document.body.removeChild(link);
                                }}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4" onClick={() => setShowForm(false)}>
          <div className="bg-white rounded-lg p-6 max-w-2xl w-full max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium mb-2" style={{ color: '#374151' }}>
                  كتاب أو أمر العضوية (PDF أو صورة)
                </label>
                <div className="space-y-2">
                  <input
                    id="membership-document"
                    type="file"
