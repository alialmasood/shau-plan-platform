...
export async function GET(request: NextRequest) {
  try {
    ...
    const result = await query(
      `SELECT 
        id, user_id, seminar_title, date, type, beneficiary_organization, location, created_at, updated_at
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { userId, seminarTitle, date, type, beneficiaryOrganization, location } = body;
    ...
    const result = await query(
      `INSERT INTO seminars (
        user_id, seminar_title, date, type, beneficiary_organization, location
      ) VALUES ($1, $2, $3, $4, $5, $6)
      RETURNING *`,
      [
...
export async function PATCH(request: NextRequest) {
  try {
    const body = await request.json();
    const { id, seminarTitle, date, type, beneficiaryOrganization, location } = body;
